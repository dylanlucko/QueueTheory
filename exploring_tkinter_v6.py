...
class GUI:
    def __init__(self, root):
        ...
        self.frame.pack(padx=10, pady=10, fill='both', expand=True)  # Set padding for the entire frame
        ...
        for i, label_text in enumerate(labels):
            ...
            label.grid(row=i, column=0, sticky="w", padx=(5, 10), pady=(5, 10))  # Adjust padx and pady here

            entry = tk.Entry(input_frame, font=("Arial", 12))
            entry.grid(row=i, column=1, padx=(5, 10), pady=(5, 10))  # Adjust padx and pady here
            self.entries[label_text] = entry

        # Calculate Button
        calculate_button = tk.Button(input_frame, text="Calculate", command=self.calculate_metrics, font=("Arial", 14, "bold"))
        calculate_button.grid(row=len(labels), columnspan=2, sticky="ew", padx=5, pady=20)  # Set columnspan to 2

        # Results Area
        self.text_area = tk.Text(self.frame, width=60, height=20, font=("Arial", 12))
        self.text_area.pack(padx=50, pady=50, side='bottom')

        # Graph Area for Plot
        self.figure, self.ax = plt.subplots(figsize=(8, 6))
        self.canvas = FigureCanvasTkAgg(self.figure, master=self.frame)
        self.canvas.get_tk_widget().pack(padx=20, pady=(10, 20), side='right')
